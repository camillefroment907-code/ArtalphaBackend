            ...
            <div className="portfolio-dark-header" style={{ background: '#0A1628', borderRadius: 12, padding: '22px 24px', marginBottom: 20 }}>
              ...
              <div className="portfolio-kpi-strip" style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', borderTop: '0.5px solid rgba(255,255,255,0.06)', paddingTop: 16 }}>
                {(() => {
                  ...
                  const kpis = [
                    { label: currentLang === 'fr' ? 'Valeur estimée' : 'Est. value', value: fmt(totalValue), sub: returnPct !== 0 ? `${returnPct > 0 ? '+' : ''}${returnPct.toFixed(1)}% ${currentLang === 'fr' ? 'vs prix saisi' : 'vs purchase price'}` : null, subColor: returnPct > 0 ? '#16A34A' : '#C0392B' },
                    ...
                    { label: currentLang === 'fr' ? 'Variation' : 'Change', value: returnPct !== 0 ? `${returnPct > 0 ? '+' : ''}${returnPct.toFixed(1)}%` : '—', sub: currentLang === 'fr' ? 'Hors frais acheteur/vente' : 'Excl. buyer/seller fees', subColor: returnPct > 0 ? '#16A34A' : 'rgba(255,255,255,0.3)', valueColor: returnPct > 0 ? '#16A34A' : 'rgba(255,255,255,0.4)' },
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1001, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '5vh', overflowY: 'auto' }}
            ...
            <div style={{ background: 'white', borderRadius: 12, width: 560, maxWidth: '92vw', maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.18)', margin: '0 auto' }}
              ...
              <div style={{ padding: '22px 26px' }}>
                <div style={{ background: 'rgba(37,99,235,0.05)', border: '0.5px solid rgba(37,99,235,0.15)', borderRadius: 8, padding: '12px 14px', marginBottom: 20 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>
                    {(() => {
                      ...
                      if (price >= 5000) return currentLang === 'fr' ? `À €${Math.round(price).toLocaleString('fr-FR')}, Artcurial et Millon sont les mieux positionnés pour cette tranche de prix, avec une forte présence auprès des collectionneurs français et européens.` : `At €${Math.round(price).toLocaleString('en')}, Artcurial and Millon are best positioned for this price range, with strong presence among French and European collectors.`;
                      return currentLang === 'fr' ? `À €${Math.round(price).toLocaleString('fr-FR')}, Millon et Drouot offrent la meilleure accessibilité et le plus grand volume d'acheteurs pour ce segment. Estimations gratuites disponibles en ligne.` : `At €${Math.round(price).toLocaleString('en')}, Millon and Drouot offer the best accessibility and buyer volume for this segment. Free estimates available online.`;
                    })()}
                  </div>
                </div>
                ...
                {(() => {
                  const price = saleModal.estimated_current_value_eur || saleModal.purchase_price_eur || 0;
                  const UTM = '?utm_source=nautilus&utm_medium=referral&utm_campaign=sell_modal';
                  const houses = price >= 100000 ? [
                    { name: "Christie's", avg: '€297 748', lots: '199 268', score: 96, url: `https://www.christies.com/selling-services/auction-estimates${UTM}`, contact: 'sellingservices@christies.com', specialty: currentLang === 'fr' ? 'Art contemporain · Blue chip · International' : 'Contemporary art · Blue chip · International', tag: currentLang === 'fr' ? 'Recommandé' : 'Recommended' },
                    { name: "Sotheby's", avg: '€225 228', lots: '291 060', score: 93, url: `https://www.sothebys.com/en/sell${UTM}`, contact: 'consign@sothebys.com', specialty: currentLang === 'fr' ? 'Art moderne · Impressionnisme · International' : 'Modern art · Impressionism · International', tag: null },
                    { name: 'Phillips', avg: '€136 986', lots: '55 220', score: 84, url: `https://www.phillips.com/sell${UTM}`, contact: 'consignments@phillips.com', specialty: currentLang === 'fr' ? 'Art contemporain · Design · Émergent' : 'Contemporary art · Design · Emerging', tag: null },
                  ] : price >= 20000 ? [
                    { name: 'Phillips', avg: '€136 986', lots: '55 220', score: 94, url: `https://www.phillips.com/sell${UTM}`, contact: 'consignments@phillips.com', specialty: currentLang === 'fr' ? 'Art contemporain · Design · Émergent' : 'Contemporary art · Design · Emerging', tag: currentLang === 'fr' ? 'Recommandé' : 'Recommended' },
                    { name: 'Bonhams', avg: '€20 063', lots: '45 586', score: 87, url: `https://sell.bonhams.com/${UTM}`, contact: 'paris.art@bonhams.com', specialty: currentLang === 'fr' ? 'Art européen · Britannique · Accessible' : 'European · British art · Accessible', tag: null },
                    { name: 'Dorotheum', avg: '€30 206', lots: '10 655', score: 79, url: `https://www.dorotheum.com/en/sell/${UTM}`, contact: 'office@dorotheum.at', specialty: currentLang === 'fr' ? 'Art européen · Vienne · Classique' : 'European art · Vienna · Classical', tag: null },
                  ] : price >= 5000 ? [
                    { name: 'Artcurial', avg: '€25 064', lots: '20 980', score: 92, url: `https://www.artcurial.com/vente-achat/vendre-avec-artcurial${UTM}`, contact: 'vendre@artcurial.com', specialty: currentLang === 'fr' ? 'Art français · Moderne · Contemporain' : 'French art · Modern · Contemporary', tag: currentLang === 'fr' ? 'Recommandé' : 'Recommended' },
                    { name: 'Millon', avg: '€8 200', lots: '35 000+', score: 86, url: `https://www.millon.com/la-vente-avec-millon${UTM}`, contact: 'contact@millon.com', specialty: currentLang === 'fr' ? 'Art français · Tableaux · Sculptures' : 'French art · Paintings · Sculptures', tag: null },
                    { name: 'Bonhams Cornette de Saint Cyr', avg: '€15 922', lots: '10 424', score: 81, url: `https://csc.bonhams.com/vendre${UTM}`, contact: 'paris@bonhams.com', specialty: currentLang === 'fr' ? 'Art contemporain français · Paris' : 'French contemporary art · Paris', tag: null },
                  ] : [
                    { name: 'Millon', avg: '€8 200', lots: '35 000+', score: 90, url: `https://www.millon.com/la-vente-avec-millon${UTM}`, contact: 'contact@millon.com', specialty: currentLang === 'fr' ? 'Art français · Accessible · Toutes spécialités' : 'French art · Accessible · All specialties', tag: currentLang === 'fr' ? 'Recommandé' : 'Recommended' },
                    { name: 'Drouot', avg: '€4 500', lots: '230 000+', score: 85, url: `https://www.drouot-estimations.com/vendre${UTM}`, contact: 'contact@drouot-estimations.com', specialty: currentLang === 'fr' ? 'Toutes spécialités · Paris · Volume' : 'All specialties · Paris · Volume', tag: null },
                    { name: 'Bonhams Cornette de Saint Cyr', avg: '€15 922', lots: '10 424', score: 78, url: `https://csc.bonhams.com/vendre${UTM}`, contact: 'paris@bonhams.com', specialty: currentLang === 'fr' ? 'Art contemporain français · Paris' : 'French contemporary art · Paris', tag: null },
                  ];
                  return (
