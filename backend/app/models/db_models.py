...
class AgentConfig(Base):
    """AI agent configuration per user (Pro+ only)."""
    ...
    # Investment criteria (copied from UserPreference at activation, can diverge)
    budget_min_eur = Column(Float, nullable=True)
    budget_max_eur = Column(Float, nullable=True)
    investment_horizon = Column(String(50), nullable=True)  # short/medium/long
    collector_type = Column(String(50), nullable=True)
    favorite_artists = Column(ARRAY(String), default=list)
    preferred_categories = Column(ARRAY(String), default=list)
    risk_tolerance = Column(String(20), default="medium")  # low/medium/high
    min_conviction_score = Column(Integer, default=70)  # 0-100, only alert above this

    # Agent behavior
    max_recommendations_per_day = Column(Integer, default=3)
...
class AgentRecommendation(Base):
    """A recommendation generated by the AI agent for a specific user×lot."""
    __tablename__ = "agent_recommendations"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"))
    lot_id = Column(UUID(as_uuid=True), ForeignKey("lots.id", ondelete="SET NULL"), nullable=True)
    agent_config_id = Column(UUID(as_uuid=True), ForeignKey("agent_configs.id", ondelete="CASCADE"))
    ...
    is_acted_on = Column(Boolean, default=False)  # user marked as purchased
    ...
    __table_args__ = (
        ...
        UniqueConstraint("user_id", "lot_id", name="uq_agent_rec_user_lot"),
    )
